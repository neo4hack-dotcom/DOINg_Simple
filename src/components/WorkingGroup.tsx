...
const WorkingGroupModule: React.FC<WorkingGroupProps> = ({ groups, users, teams, currentUser, llmConfig, onUpdateGroup, onDeleteGroup }) => {
    ...
    const [activeTab, setActiveTab] = useState<'notes' | 'actions' | 'checklist'>('notes');
    ...
    const handleEditSession = (groupId: string, session: WorkingGroupSession) => {
        ...
        // This ensures "Cancel" works correctly and prevents React rendering bugs
    ...
    const updateAction = (index: number, field: keyof ActionItem, value: any) => {
        ...
        const newActions = editingSession.session.actionItems.map((item, i) => 
    ...
    const addAction = () => {
        ...
        setEditingSession({
            ...editingSession,
            session: { ...editingSession.session, actionItems: [...editingSession.session.actionItems, newAction] }
        });
    };
                    ...
                    <div className="p-6 overflow-y-auto flex-1 bg-slate-50/50 dark:bg-slate-900/50">
                        ...
                        {activeTab === 'actions' && (
                            <div className="space-y-4">
                                <button onClick={addAction} className="w-full py-2 border-2 border-dashed border-indigo-200 dark:border-indigo-900/50 rounded-xl text-indigo-600 dark:text-indigo-400 font-bold text-sm hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-colors flex items-center justify-center gap-2">
                                    <Plus className="w-4 h-4" /> Add Action Item
                                </button>

                                <div className="space-y-3">
                                    {session.actionItems.map((action, idx) => {
                                        const isFreeText = action.ownerId && !users.some(u => u.id === action.ownerId);
                                        return (
                                            <div key={action.id} className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col gap-3">
                                                <div className="flex gap-2">
                                                    <input 
                                                        ...
                                                        className="flex-1 bg-transparent border-b border-transparent hover:border-slate-200 focus:border-indigo-500 outline-none font-medium text-slate-800 dark:text-white"
                                                    />
                                                    <button onClick={() => deleteAction(idx)} className="text-slate-400 hover:text-red-500"><Trash2 className="w-4 h-4"/></button>
                                                </div>
                                                ...
                                                <div className="flex flex-wrap gap-2 items-center">
                                                    ...
                                                    <select 
                                                        value={action.status}
                                                        onChange={(e) => updateAction(idx, 'status', e.target.value as ActionItemStatus)}
                                                        className={`text-xs font-bold px-2 py-1.5 rounded border-none cursor-pointer ${getStatusColor(action.status)}`}
                                                    >
                                                    ...
                                                    <div className="relative min-w-[120px]">
                                                        {isFreeText ? (
                                                            <div className="flex items-center">
                                                                ...
                                                                />
                                                                <button onClick={() => updateAction(idx, 'ownerId', '')} className="ml-1 text-slate-400 hover:text-red-500"><X className="w-3 h-3" /></button>
                                                            </div>
                                                        ) : (
                                                            <select 
                                                                value={action.ownerId}
                                                                onChange={(e) => e.target.value === 'FREE_TEXT' ? updateAction(idx, 'ownerId', 'External') : updateAction(idx, 'ownerId', e.target.value)}
                                                                className="w-full text-xs p-1.5 rounded border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700 dark:text-white"
                                                            >
                                                                <option value="">Assignee...</option>
                                                                {users.map(u => <option key={u.id} value={u.id}>{u.firstName} {u.lastName}</option>)}
                                                                <option value="FREE_TEXT">-- External --</option>
                                                            </select>
                                                        )}
                                                    </div>
                                                    ...
                                                    <input 
                                                        ...
                                                        className="w-24 text-xs p-1.5 rounded border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700 dark:text-white placeholder-slate-400"
                                                    ...
                                                    <select 
                                                        ...
                                                        className="text-xs p-1.5 rounded border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700 dark:text-white"
                                                    >
                                                    ...
                                                    <input 
                                                        type="date"
                                                        value={action.eta || ''}
                                                        onChange={(e) => updateAction(idx, 'eta', e.target.value)}
                                                        className="text-xs p-1.5 rounded border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700 dark:text-white text-slate-500"
                                                    />
                                                </div>
                                            </div>
                                        )
