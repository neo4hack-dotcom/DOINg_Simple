...
const WorkingGroupModule: React.FC<WorkingGroupProps> = ({ groups, users, teams, currentUser, llmConfig, onUpdateGroup, onDeleteGroup }) => {
    ...
    const [activeTab, setActiveTab] = useState<'notes' | 'actions' | 'checklist' | 'decisions'>('notes');
    ...
    const addDecision = () => {
        if (!editingSession) return;
        const newDecision: Decision = { id: generateId(), text: '' };
        setEditingSession({
            ...editingSession,
            session: { ...editingSession.session, decisions: [...(editingSession.session.decisions || []), newDecision] }
        });
    }

    const updateDecision = (index: number, text: string) => {
        if (!editingSession) return;
        const newDecisions = (editingSession.session.decisions || []).map((d, i) => 
            i === index ? { ...d, text } : d
        );
        setEditingSession({
            ...editingSession,
    ...
    const deleteDecision = (index: number) => {
        if (!editingSession) return;
        const newDecisions = (editingSession.session.decisions || []).filter((_, i) => i !== index);
        setEditingSession({
            ...editingSession,
            session: { ...editingSession.session, decisions: newDecisions }
        });
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-4xl flex flex-col h-[85vh] border border-slate-200 dark:border-slate-700 overflow-hidden animate-in zoom-in-95 duration-200">
                    ...
                    <div className="flex border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-x-auto">
                        <button 
                            ...
                            className={`flex-1 py-3 text-sm font-bold border-b-2 transition-colors min-w-[80px] ${activeTab === 'notes' ? 'border-indigo-600 text-indigo-600 dark:text-indigo-400' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
                        >
                            Notes
                        </button>
                        <button 
                            onClick={() => setActiveTab('decisions')}
                            className={`flex-1 py-3 text-sm font-bold border-b-2 transition-colors min-w-[100px] ${activeTab === 'decisions' ? 'border-indigo-600 text-indigo-600 dark:text-indigo-400' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
                        >
                            Decisions ({(session.decisions || []).length})
                        </button>
                        <button 
                            onClick={() => setActiveTab('actions')}
                            className={`flex-1 py-3 text-sm font-bold border-b-2 transition-colors min-w-[100px] ${activeTab === 'actions' ? 'border-indigo-600 text-indigo-600 dark:text-indigo-400' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
                        >
                            Actions ({session.actionItems.length})
                        </button>
                        <button 
                            onClick={() => setActiveTab('checklist')}
                            className={`flex-1 py-3 text-sm font-bold border-b-2 transition-colors min-w-[100px] ${activeTab === 'checklist' ? 'border-indigo-600 text-indigo-600 dark:text-indigo-400' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
                        >
                    ...
                    <div className="p-6 overflow-y-auto flex-1 bg-slate-50/50 dark:bg-slate-900/50">
                        ...
                        {activeTab === 'decisions' && (
                            <div className="space-y-4">
                                <button onClick={addDecision} className="w-full py-2 border-2 border-dashed border-amber-200 dark:border-amber-900/50 rounded-xl text-amber-600 dark:text-amber-400 font-bold text-sm hover:bg-amber-50 dark:hover:bg-amber-900/20 transition-colors flex items-center justify-center gap-2">
                                    <Gavel className="w-4 h-4" /> Add Decision
                                </button>

                                <div className="space-y-2">
                                    {(session.decisions || []).map((decision, idx) => (
                                        <div key={decision.id} className="flex items-start gap-3 bg-white dark:bg-slate-800 p-3 rounded-lg border border-slate-200 dark:border-slate-700">
                                            ...
                                            />
                                            <button onClick={() => deleteDecision(idx)} className="text-slate-300 hover:text-red-500 pt-0.5"><Trash2 className="w-4 h-4"/></button>
                                        </div>
                                    ))}
                                    {(session.decisions || []).length === 0 && <p className="text-center text-sm text-slate-400 italic py-4">No decisions recorded.</p>}
                                </div>
                            </div>
                        )}
                                            ...
                                            {session.decisions && session.decisions.length > 0 && (
                                                <div className="mb-4 space-y-1">
                                                    {session.decisions.map((decision) => (
                                                        <div key={decision.id} className="flex items-start gap-2 text-xs bg-amber-50 dark:bg-amber-900/20 p-2 rounded text-slate-700 dark:text-slate-200 border border-amber-100 dark:border-amber-900/30">
                                                            <Gavel className="w-4 h-4 text-amber-500 flex-shrink-0 mt-0.5" />
